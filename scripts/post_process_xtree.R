...
COVERAGE_CUTOFF <- 0.1
GTDBFILELOC <- "bac120_taxonomy.tsv"
###
...
### EXAMPLE AS TO HOW YOU COULD MERGE IN GTDB TAXONOMY
taxonomy_df <- read.delim(GTDBFILELOC, header = FALSE, stringsAsFactors = FALSE)
...
all_data$RA[is.na(df$RA)] <- 0
write.csv(all_data, "merged_xtree.csv")
